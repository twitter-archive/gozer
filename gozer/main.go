...
func main() {
	...
	for {
		...
		case update := <-driver.Updates:
			...
			if err != nil {
				log.Printf("Gozer: failed to get current state for updated task %q: %+s", update.TaskId, err)
				continue
			}

			newState, ok := taskStateMap[update.State]
			if !ok {
				log.Printf("Gozer: unknown mesos task state: %s", update.State)
				continue
			}

			log.Println("Gozer: updating task state from", state, "to", newState)
